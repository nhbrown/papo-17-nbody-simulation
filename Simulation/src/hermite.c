...
{ 
 ...
 for(int i = 0; i < N; ++i) /* loops over all particles */
 {
   ...
   {
     double complex rji[3]; /* position vector from particle i to j */
     double complex vji[3]; /* velocity vector from particle i to j */
             ...
             double complex **vel, double complex **acc, double complex **jerk)
{
  ...
  double complex old_pos[N][3];
  double complex old_vel[N][3];  
  double complex old_acc[N][3];
  double complex old_jerk[N][3];
  ...
  acc_jerk(N, mass, pos, vel, acc, jerk); /* get the new accleration and jerk for all particles*/
                  ...
                  double complex **acc, double complex **jerk)
{
  ...
  acc_jerk(N, mass, pos, vel, acc, jerk); /* one time calculation to get inital acceleration and jerk for all particles */
  ...
  while(time < end_time) /* until user specified end of simulation is reached */
  {
    ...
    hermite(N, dt, mass, pos, vel, acc, jerk); /* calculate movement for current iteration */
